...
// ── BAGS CHECKOUT PAGE ──────────────────────────────────────────────────────
export default function BagsCheckoutPage({cart,user,L,setPage,onComplete,toast,mobile}) {
  ...
  // Bag icon
  const BagIcon = ({size=24,color=C.tan})=>(
            ...
            {confirmedItems.map((o,i)=>(
              <div key={i} style={{display:"flex",gap:14,padding:"14px 0",borderBottom:i<confirmedItems.length-1?`1px solid rgba(200,200,190,0.3)`:"none"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <p style={{...T.bodySm,color:C.gray,fontSize:11}}>{o.color}</p>
                </div>
      ...
      <div style={{background:C.white,borderBottom:`1px solid ${C.lgray}`,position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:mobile?"14px 16px":"18px 40px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:mobile?6:10}}>
            <BagIcon size={mobile?16:18} color={C.tan}/>
            <span style={{...T.labelSm,fontSize:mobile?9:11,color:C.tan,letterSpacing:"0.12em"}}>{L?.bagsCheckout||"BAGS CHECKOUT"}</span>
          </div>
        ...
        <div>
          {step===1&&(
            <>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:6}}>
                <BagIcon size={28} color={C.black}/>
                <h2 style={{fontFamily:"'Alido',serif",fontSize:mobile?24:30,fontWeight:300,color:C.black}}>{L?.bagsShippingDetails||"Bags — Shipping Details"}</h2>
              </div>
              <p style={{...T.bodySm,color:C.gray,fontSize:13,marginBottom:28}}>{L?.bagsShippingSubtitle||"Complete your designer bag order"}</p>
              ...
              {isGuest&&(
                <>
                  <div style={{padding:"14px 18px",background:"rgba(177,154,122,0.05)",border:`1px solid rgba(177,154,122,0.12)`,marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <button onClick={()=>{window.__returnAfterAuth="checkout-bags";setPage("auth");}} style={{background:"none",border:"none",...T.label,color:C.tan,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>{L?.signInBtn||"Sign In"}</button>
                  </div>
          ...
          {step===2&&(
            <>
              ...
              <p style={{...T.bodySm,color:C.gray,fontSize:13,marginBottom:28}}>{L?.reviewSubtitle||"Please review your bag order before confirming"}</p>
          ...
          <div style={{position:mobile?"relative":"sticky",top:mobile?"auto":100}}>
            <div style={{background:C.white,padding:mobile?"24px 20px":"28px 24px"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:18}}>
                <BagIcon size={16} color={C.tan}/>
                <p style={{...T.labelSm,color:C.tan,fontSize:9,letterSpacing:"0.15em"}}>{L?.bagsOrder||"BAGS ORDER"} ({cart.length})</p>
              </div>
              ...
              {cart.map((o,i)=>(
                <div key={o.addedAt||i} style={{display:"flex",gap:12,padding:"12px 0",borderBottom:`1px solid rgba(200,200,190,0.3)`}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <p style={{...T.bodySm,color:C.gray,fontSize:10}}>{o.color}</p>
                  </div>
            ...
            <div style={{padding:"18px 24px",background:"rgba(177,154,122,0.04)",marginTop:2}}>
              {[
                {text:L?.authenticityGuaranteed||"Authenticity guaranteed — verified suppliers"},
