    ...
    <div style={{paddingTop:mobile?52:80,background:C.cream}}>
      ...
      <div style={{padding:"64px 0",borderBottom:`1px solid ${C.lgray}`}}>
        <div style={{maxWidth:900,margin:"0 auto",padding:mobile?"0 16px":"0 40px",display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:mobile?20:52}}>
          <p style={{...T.body,color:C.gray,lineHeight:1.9}}>{L&&L.aboutBody1||'Walk into a Georgian shop and you pay for the shopkeeper\'s rent. Order online from an unverified source and receive something that looked premium in the photo but felt wrong in your hands.'}</p>
          <p style={{...T.body,color:C.gray,lineHeight:1.9}}>{L&&L.aboutBody2||'Neither option respected the customer. ALTERNATIVES was built to offer exactly that — a simple, honest alternative to a market that had stopped trying.'}</p>
        </div>
      </div>
      <div style={{padding:"64px 0",background:C.offwhite}}>
        <div style={{maxWidth:1360,margin:"0 auto",padding:mobile?"0 16px":"0 40px",display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:mobile?28:64,alignItems:"center"}}>
          ...
          <div>
            ...
            <HoverBtn onClick={()=>setPage("how")} variant="primary">{L&&L.ourProcess||'Our Process'} →</HoverBtn>
          </div>
        </div>
      </div>
