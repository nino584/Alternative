    ...
    <div style={{background:C.cream}}>
      ...
      <section style={mobile?{minHeight:"100vh",display:"flex",flexDirection:"column",overflow:"hidden",position:"relative",paddingTop:60}:{height:"100vh",display:"grid",gridTemplateColumns:"1fr 1fr",overflow:"hidden",minHeight:600,position:"relative"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",justifyContent:mobile?"flex-start":"flex-end",padding:mobile?"32px 20px 40px":"0  clamp(32px,5vw,60px) 80px",position:"relative",zIndex:1,flex:mobile?1:undefined,background:C.heroBg}}>
          <div style={{...f(0)}}>
            <p style={{fontFamily:"'TT Interphases Pro',sans-serif",fontSize:14,fontWeight:400,letterSpacing:"0.12em",textTransform:"uppercase",color:C.heroSub,marginBottom:mobile?14:20,display:"flex",alignItems:"center",gap:10}}><span style={{display:"inline-block",width:24,height:1,background:C.heroSub}}/>{L.heroSub}</p>
          </div>
          <div style={f(0.15)}>
        ...
        {!mobile&&(
          <div style={{position:"relative",overflow:"hidden",background:C.heroRightBg}}>
            <img src={heroSrc} alt="Alternative luxury pre-order fashion" fetchpriority="high" onError={()=>setHeroSrc(BI.bag_stone)} style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"center center",opacity:vis?1:0,transform:vis?"scale(1)":"scale(1.04)",transition:"opacity 1.2s ease, transform 1.4s ease"}}/>
          ...
          <div style={mobile?{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}:{display:"grid",gridTemplateColumns:"repeat(4,1fr)"}}>
            {[
              ...
              <div key={i} style={{padding:mobile?"14px 12px":"22px 20px",borderRight:mobile?(i%2===0?`1px solid rgba(177,154,122,0.12)`:"none"):(i<3?`1px solid rgba(177,154,122,0.12)`:"none"),borderBottom:mobile&&i<2?`1px solid rgba(177,154,122,0.12)`:"none",display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{...T.bodySm,color:C.onDark,lineHeight:1.5}}>{item.t}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={featRef} className={`reveal${featVis?' visible':''}`} style={{padding:mobile?"32px 0 28px":"64px 0 56px"}}>
        <div style={{maxWidth:1360,margin:"0 auto",padding:`0 ${px}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:mobile?28:44}}>
            ...
            <button onClick={()=>setPage("catalog")} style={{background:"none",border:"none",...T.bodySm,color:C.gray,textDecoration:"underline",cursor:"pointer"}}>View all →</button>
          </div>
            ...
            {[{name:L.womenswear,section:"Womenswear",src:BI.bag_stone,sub:L.womensSub},{name:L.menswear,section:"Menswear",src:BI.man_editorial,sub:L.mensSub},{name:L.kidswear,section:"Kidswear",src:BI.packaging,sub:L.kidsSub}].map((cat,i)=>(
              <div key={i} onClick={()=>{window.__initSection=cat.section;setPage("catalog");}} style={{position:"relative",height:mobile?260:420,cursor:"pointer",overflow:"hidden"}}>
                ...
                <div style={{position:"absolute",bottom:mobile?16:24,left:mobile?16:24}}>
                  ...
                  <p style={{...T.labelSm,color:"rgba(255,255,255,0.85)"}}>{cat.sub}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section ref={vidRef} className={`reveal${vidVis?' visible':''}`} style={{background:C.black,padding:mobile?"48px 0":"72px 0"}}>
        <div style={{maxWidth:1360,margin:"0 auto",padding:`0 ${px}`,display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:mobile?32:72,alignItems:"center"}}>
          <div style={{position:"relative",height:mobile?220:380,overflow:"hidden",order:mobile?1:0,background:C.black}}>
            {!vidPlaying?(
              <>
                <img src={BI.store_interior} alt="Video verification service at Alternative" loading="lazy" style={{width:"100%",height:"100%",objectFit:"cover",opacity:0.6}}/>
                <button onClick={()=>setVidPlaying(true)} aria-label="Play demo video" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:60,height:60,borderRadius:"50%",border:"2px solid rgba(255,255,255,0.85)",background:"rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all 0.3s",backdropFilter:"blur(4px)"}}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="rgba(255,255,255,0.95)"><polygon points="6 3 20 12 6 21 6 3"/></svg>
                </button>
                <p style={{position:"absolute",bottom:mobile?12:20,left:0,right:0,textAlign:"center",...T.labelSm,color:"rgba(255,255,255,0.8)",fontSize:9,letterSpacing:"0.15em"}}>WATCH SAMPLE VERIFICATION</p>
              </>
            ):(
          ...
          <div style={{order:mobile?0:1}}>
            <span style={{...T.labelSm,display:"inline-block",color:C.tan,marginBottom:mobile?12:18,padding:"4px 12px",border:`1px solid ${C.tan}`}}>{L.extraService}</span>
            ...
            <p style={{...T.body,color:C.onDark,lineHeight:1.9,marginBottom:mobile?24:36}}>{L.videoBody}</p>
            <HoverBtn onClick={()=>setPage("video-verification")} variant="tan">{L.shopAddService}</HoverBtn>
          </div>
        </div>
      </section>

      <section ref={qualRef} className={`reveal${qualVis?' visible':''}`} style={{background:C.black,padding:mobile?"48px 0":"72px 0",borderTop:`1px solid rgba(177,154,122,0.1)`}}>
        <div style={{maxWidth:1360,margin:"0 auto",padding:`0 ${px}`,display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:mobile?32:72,alignItems:"center"}}>
          <div>
            ...
            <p style={{...T.body,color:C.onDark,lineHeight:1.9,marginBottom:mobile?24:36}}>{L.verifyBody||"Our dedicated team physically handles every piece — checking hardware, stitching, and material quality — before it ships. What we verify is what arrives."}</p>
            <div style={{display:"flex",gap:mobile?24:40}}>
              {[["50+",L.itemsVerified],["98%",L.custSatisfaction],["4.9",L.avgRating]].map(([n,l])=>(
                <div key={n}><p style={{fontFamily:"'Alido',serif",fontSize:mobile?24:30,fontWeight:300,color:C.tan,lineHeight:1}}>{n}</p><p style={{...T.labelSm,color:C.onDark,marginTop:5}}>{l}</p></div>
              ))}
            </div>
          </div>
