  ...
  return (
    <div style={{ background: C.cream, marginBottom: 40, animation: "fadeUp 0.3s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", borderBottom: `1px solid ${C.lgray}` }}>
        <p style={{ ...T.label, color: C.black, fontSize: 12 }}>Promo Codes ({promos.length})</p>
        <div style={{ display: "flex", gap: 8 }}>
          {promos.length > 0 && <HoverBtn onClick={exportCSV} variant="ghost" style={{ padding: "8px 14px", fontSize: 9 }}>Export CSV</HoverBtn>}
      ...
      {showForm && (
        <div style={{ padding: 20, borderBottom: `1px solid ${C.lgray}`, background: C.offwhite, animation: "slideDown 0.2s ease" }}>
          <p style={{ ...T.label, color: C.black, fontSize: 11, marginBottom: 16 }}>Create Promo Code</p>
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr 1fr", gap: 14 }}>
            <div>
              <label style={{ ...T.labelSm, color: C.gray, display: "block", marginBottom: 6 }}>CODE *</label>
              <input style={inputStyle} value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} placeholder="e.g. WELCOME20" />
            </div>
            <div>
              <label style={{ ...T.labelSm, color: C.gray, display: "block", marginBottom: 6 }}>TYPE</label>
            ...
            <div>
              <label style={{ ...T.labelSm, color: C.gray, display: "block", marginBottom: 6 }}>VALUE * ({form.type === "percent" ? "%" : "GEL"})</label>
              <input style={inputStyle} type="number" value={form.value} onChange={e => setForm(f => ({ ...f, value: e.target.value }))} placeholder="e.g. 20" />
            </div>
            <div>
              <label style={{ ...T.labelSm, color: C.gray, display: "block", marginBottom: 6 }}>MIN ORDER (GEL)</label>
            ...
            <div>
              <label style={{ ...T.labelSm, color: C.gray, display: "block", marginBottom: 6 }}>MAX USES</label>
            ...
            <div>
              <label style={{ ...T.labelSm, color: C.gray, display: "block", marginBottom: 6 }}>EXPIRES</label>
              <input style={inputStyle} type="date" value={form.expiresAt} onChange={e => setForm(f => ({ ...f, expiresAt: e.target.value }))} />
            </div>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <HoverBtn onClick={save} variant="tan" style={{ padding: "10px 28px", fontSize: 9 }}>Create Promo</HoverBtn>
            <HoverBtn onClick={() => setShowForm(false)} variant="ghost" style={{ padding: "10px 20px", fontSize: 9 }}>Cancel</HoverBtn>
          </div>
        </div>
      )}
      ...
      {promos.length === 0 ? (
        <div style={{ padding: "48px 20px", textAlign: "center" }}>
          <p style={{ ...T.bodySm, color: C.gray, marginBottom: 16 }}>No promo codes yet</p>
          <HoverBtn onClick={() => setShowForm(true)} variant="tan" style={{ padding: "10px 24px", fontSize: 9 }}>Create First Promo</HoverBtn>
        </div>
      ) : (
            ...
            <tbody>
              {promos.map(p => (
                <tr key={p.code} style={{ borderBottom: `1px solid ${C.lgray}`, transition: "background 0.15s" }}
                  ...
                  <td style={{ ...T.bodySm, color: C.gray, padding: "12px 14px" }}>{p.type === "percent" ? "Percent" : "Fixed"}</td>
                  ...
                  <td style={{ ...T.bodySm, color: C.gray, padding: "12px 14px", fontSize: 11 }}>{p.expiresAt ? new Date(p.expiresAt).toLocaleDateString() : "Never"}</td>
                  <td style={{ padding: "12px 14px" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      <HoverBtn onClick={() => toggleActive(p)} variant="ghost" style={{ padding: "5px 12px", fontSize: 10 }}>{p.active ? "Disable" : "Enable"}</HoverBtn>
                      {deleteConfirm === p.code ? (
                        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                          <span style={{ ...T.labelSm, color: C.red, fontSize: 10 }}>Sure?</span>
                          <button onClick={() => deletePromo(p.code)} style={{ background: "none", border: "none", cursor: "pointer", ...T.labelSm, color: C.tan, fontSize: 10 }}>Yes</button>
                          <button onClick={() => setDeleteConfirm(null)} style={{ background: "none", border: "none", cursor: "pointer", ...T.labelSm, color: C.red, fontSize: 10 }}>No</button>
                        </div>
                      ) : (
                        <HoverBtn onClick={() => setDeleteConfirm(p.code)} variant="danger" style={{ padding: "5px 12px", fontSize: 10 }}>Delete</HoverBtn>
                      )}
                    </div>
                  </td>
                </tr>
