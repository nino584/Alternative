  ...
  return (
    <div style={{ background: C.cream, marginBottom: 40, animation: "fadeUp 0.3s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", borderBottom: `1px solid ${C.lgray}` }}>
        <p style={{ ...T.label, color: C.black, fontSize: 12 }}>Return Requests ({returns.length})</p>
        {returns.length > 0 && <HoverBtn onClick={exportCSV} variant="ghost" style={{ padding: "8px 14px", fontSize: 9 }}>Export CSV</HoverBtn>}
      </div>

      {returns.length === 0 ? (
        <div style={{ padding: "48px 20px", textAlign: "center" }}>
          <p style={{ ...T.bodySm, color: C.gray }}>No return requests yet</p>
        </div>
      ) : (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${C.lgray}`, background: C.offwhite }}>
                {["ID", "Order", "Reason", "Status", "Date", "Actions"].map(h => (
